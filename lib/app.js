...
module.exports = Stex.new(__dirname + "/..", function(stex) {
  ...
  router.post('/v2/wallets/show', function(req, res, next) {
    lockout.ensureAllowed(req.ip)
      .then(function() {
        return walletV2.getWithAuthorization(req.body.username, req.body.authToken, req.body.totpToken);
      })
      .then(function(wallet) {
  ...
  router.post('/v2/wallets/create', function(req, res, next) {
    Promise.resolve(req.body)
      ...
      .then(validate.present("updateKey"))
      .then(validate.hash("mainData"))
      .then(validate.hash("keychainData"))
      .then(function (params) {
        return walletV2.create(params);
      })
      .then (function(wallet) { 
        res.send({"status" : "success"}); 
      })
      .catch(validate.errors.InvalidHash, function(e) {
        fieldFail(e.field, "invalid_hash");
      })
      .catch(validate.errors.MissingField, function(e) {
        fieldFail(e.field, "missing");
      })
      .catch(errors.DuplicateRecord, function(e) {
        fieldFail("username", "already_taken");
      });

    function fieldFail(field, code) {
      res.status(400).send({status: "fail", field: field, code: code });
    }
  });
