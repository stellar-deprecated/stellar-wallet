...
wallet.get = function(id) {
  ...
  return hash.locator(id).then(function(hashedId) {
    ...
    var query = db("wallets").where({ id: hashedId });

    if(conf.get("allowV2") === true) {
      query = query.where({ migratedAt:null });
    }

    return query.select().then(_.first);
  });
};

wallet.getByRecoveryId = function(recoveryId) {
  return hash.locator(recoveryId).then(function(hashedId) {
    return db("wallets").where({recoveryId: hashedId, migratedAt:null}).select().then(function(results) {
      return results[0];
    });
  });
};
...
wallet.markMigrated = function(id, authToken) {
  return wallet.get(id)
    .then(function(w) {
