...
walletV2.create = function(attrs) {
  ...
  return Promise.resolve(attrs)
    ...
    .tap(function(attrs) {
      if ('development' === process.env.NODE_ENV && _.isEmpty(attrs.usernameProof)) {
        return attrs;
      } else {
    ...
    .then(function(attrs) {
      attrs          = _.omit(attrs, ['mainDataHash', 'keychainDataHash', 'usernameProof']);
...
walletV2.initiateTotpGracePeriod = function(id) {
  return db("wallets_v2")
    .where({id: id})
    .update({totpDisabledAt: new Date()})
    .then(function () {
...
walletV2.isTotpEnabled = function(wallet) {
  ...
  if (wallet.totpDisabledAt instanceof Date) {
    ...
    var disabledAt = wallet.totpDisabledAt.getTime();
