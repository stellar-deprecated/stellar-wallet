...
/**
 * The loginFailureTracker is a module used to record failed login attempts, 
...
loginFailureTracker.errors = {};
loginFailureTracker.errors.Disabled = Error.subclass("loginFailureTracker.Disabled");
...
loginFailureTracker.record = function(id) {
  ...
  return Promise.join(
    ...
    var failedAttempts     = parseInt(results[0]);
...
loginFailureTracker.ensureAllowed = function(id) {
  return loginFailureTracker.isLoginAllowed(id)
    .then(function(isAllowed) {
      if(!isAllowed) {
        throw new loginFailureTracker.errors.Disabled();
      } 
    });
};
